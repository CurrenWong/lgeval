...
def translateStructure(lg, label, nodeRelationPairs, structureMap,
    ...
    strString = ""
    def byValue(pair): return pair[1]
...
def translateRelation(lg, relation_nextChildId, structureMap,
                      segPrimMap, edgeMap, symbolMap, nodeString):
    """Translate an individual spatial relation."""
    relation, nextChildId = relation_nextChildId
    relString = ""
    replacementTuple = ()
    ...
    if relation in structureMap.keys():
        ...
        sys.stderr.write(
            "  !! Error: Unknown relationship label " + relation + "\n")
        sys.stderr.write("  !!        Using relationship mapping: " +
    ...
    for i in range(0, len(replacementTuple)):
        ...
        if nextEntry == "PARENT":
            ...
            relString += translate(lg, nextChildId,
                                   segPrimMap, edgeMap, symbolMap, structureMap)
        else:
...
def main():
    ...
    if len(rootNodes) < 1:
        ...
        sys.stderr.write("  !! Graph contains " +
                         str(len(rootNodes)) + " root nodes.\n")
